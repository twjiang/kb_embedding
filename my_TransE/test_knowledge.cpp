...
{
    FILE *train_file = fopen("../data/train.txt","r");
...
Others:
*************************************************/
void load_valid_data()
{
    FILE *valid_file = fopen("../data/valid.txt","r");
    
    const char * split = "\t";
    char * p;
    while(!feof(valid_file))
    {
        fgets(buf, sizeof(buf), valid_file); 
...
void load_test_data()
{
    FILE *test_file = fopen("../data/test.txt","r");
